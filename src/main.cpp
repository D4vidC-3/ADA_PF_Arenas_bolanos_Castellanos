    ...
    #include "graph.hpp"   //Estructura del grafo ponderado de 20 nodos y Union-Find para Kruskal
    #include "kruskal.hpp" //Algoritmo de Kruskal para construir el MST de minimo costo
    #include "knapsack.hpp" //Mochila 0-1 por tabulacion para asignacion optima de ancho de banda
...
// ── MÓDULO B: Kruskal ──────────────────────────────────────────────────
// Calculamos el promedio de MonthlyCharges por grupo (i mod 20)
std::vector<double> promedios(20, 0.0);
std::vector<int> conteos(20, 0);
for (int i = 0; i < (int)solicitudes.size(); i++) {
    int grupo = i % 20;
    promedios[grupo] += solicitudes[i].monthlyCharges;
    conteos[grupo]++;
}
for (int k = 0; k < 20; k++) {
    if (conteos[k] > 0)
        promedios[k] = std::round(promedios[k] / conteos[k] * 100.0) / 100.0;
}

// Construimos el grafo completo de 20 nodos y 190 aristas
std::vector<Arista> aristas = construirGrafo(promedios);

// Ejecutamos Kruskal y obtenemos el MST
int pesoTotal = 0;
std::vector<Arista> mst = kruskal(20, aristas, pesoTotal);

// Escribimos el resultado del MST en results/mst_red.txt
std::ofstream mstFile("results/mst_red.txt");
mstFile << "MODULO B - MST por Kruskal\n\n";
mstFile << "Nodos: 20 | Aristas en MST: " << mst.size() << "\n";
mstFile << "Peso total del MST: " << pesoTotal << "\n\n";
mstFile << "Aristas seleccionadas:\n";
for (const Arista& a : mst) {
    mstFile << "Nodo " << a.u << " -- Nodo " << a.v << " | peso: " << a.peso << "\n";
}
mstFile.close();
std::cout << " - results/mst_red.txt\n";

// ── MÓDULO C: Knapsack ─────────────────────────────────────────────────
// Toma las 50 solicitudes activas (Churn=No) de mayor tenure del arreglo ordenado
// y aplica Mochila 0-1 con W=500 para maximizar ingreso
ejecutarModuloC(solicitudes);
std::cout << " - results/asignacion_bw.txt\n";
    }